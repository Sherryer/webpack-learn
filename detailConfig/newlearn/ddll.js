...
function dispatchQueue(loader) {                                                                // 把实例上的 _queue 卸载
    ...
    if (maxBatchSize && maxBatchSize > 0 && maxBatchSize < queue.length) {                      // 分批 触发 dispatchQueueBatch
...
function dispatchQueueBatch(loader, queue) {
    ...
    var keys = queue.map(function (_ref) {                                                      // 收集所有 keys
        var key = _ref.key;
        return key;
    });
    ...
    var batchPromise = batchLoadFn(keys);                                                       // 把 keys 给 callback，获取 promise
    ...
    batchPromise.then(function (values) {
        ...
        // loaded queue.
        queue.forEach(function (_ref2, index) {                                                 // 做 id 和数据的 一一对应
...
function failedDispatch(loader, queue, error) {                                                 // 遍历 queue，依次从 _promiseCache 中清楚数据，依次 reject
